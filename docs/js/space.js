...
export function init(scene) {
  ...
  const wingFrameMat = getMaterial('solarFrame');
  const wingCellMat = getMaterial('satelliteSolarCells');
  ...
  const beaconR = new THREE.Mesh(new THREE.SphereGeometry(4, 12, 12), getMaterial('ledRed'));
  beaconR.position.set(-290, 0, 0);
  satGroup.add(beaconR);

  const beaconG = new THREE.Mesh(new THREE.SphereGeometry(4, 12, 12), getMaterial('ledGreen'));
...
export function triggerSpaceTransmission() {
  pulseSpeed = 0.025;
...
export function getSatellite() { return satelliteMesh; }
