...
const ModelsBuilder = (function () {
  ...
  function buildRackEnclosure() {
    ...
    const alumMat = MaterialsRegistry.get('aluminum');
    const railMat = MaterialsRegistry.get('steelRail');
    ...
    const doorMesh = new THREE.Mesh(
      ...
      MaterialsRegistry.get('glassDoor')
    );
    ...
    const handle = new THREE.Mesh(
      ...
      MaterialsRegistry.get('aluminum')
    );
  ...
  function buildRackMountComponents() {
    const chassisMat = MaterialsRegistry.get('chassisBody');
    const uHeight = 44.45;
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const led = new THREE.Mesh(
        ...
        i % 2 === 0 ? MaterialsRegistry.get('ledGreen') : MaterialsRegistry.get('ledBlue')
      );
    ...
    [-80, 80].forEach((x) => {
      const heatsink = new THREE.Mesh(
        ...
        MaterialsRegistry.get('goldHeatsink')
      );
      ...
      const block = new THREE.Mesh(
        ...
        MaterialsRegistry.get('copperBlock')
      );
    ...
    const statusLed1 = new THREE.Mesh(
      ...
      MaterialsRegistry.get('ledGreen')
    );
  ...
  function buildCoolingLoop() {
    const tubeMat = MaterialsRegistry.get('coolantTube');
  ...
  function buildPowerWall() {
    ...
    const invBody = new THREE.Mesh(
      ...
      MaterialsRegistry.get('victronBlue')
    );
  ...
  function buildSolarArray() {
    ...
    const frameMat = MaterialsRegistry.get('solarFrame');
    const cellMat = MaterialsRegistry.get('solarCells');
  ...
  function buildSpaceTerminal() {
    ...
    const bodyMat = MaterialsRegistry.get('spaceTerminalBody');
    ...
    const apertureFace = new THREE.Mesh(
      ...
      MaterialsRegistry.get('phasedArrayAperture')
    );
  ...
  function toggleDoor() {
    ...
    if (window.TWEEN) {
      ...
      doorPivot.rotation.y = targetRot;
    }
    return doorOpen;
  }

  function setExplodedView(progress) {
  ...
  function setSubsystemVisibility(layerKey) {
  ...
  return {
    ...
    getComponentData: (key) => COMPONENT_DATA[key],
  };
})();
